...
const MAGraph: React.FC<MAGraphProps> = ({
  data,
  title = "2Y Moving Average Multiplier",
  ...
  useEffect(() => {
    ...
    const pricePath = g.append('path')
      ...
      .attr('clip-path', 'url(#magraph-clip)');
  ...
  const captureChart = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        ...
        link.download = `magraph-${new Date().getTime()}.png`;
